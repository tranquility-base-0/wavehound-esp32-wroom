...
// ---------------------------------------------------------------------------
#define PROF_TIME(call, us_tot, us_max, n)                                     \
  do {                                                                         \
    uint32_t _t0 = micros();                                                   \
    call;                                                                      \
...
void setup() {
  ...
  Serial.printf("PacketCapture=%u bytes | leakQueue storage ~%u bytes\n",
  ...
  liveDumpQueue = xQueueCreate(LIVE_DUMP_QUEUE_DEPTH, sizeof(LiveCaptureEvent));
...
void loop() {
  ...
  if (millis() - last_debug_print > 3000) {
    ...
    Serial.printf("[PROF] PEND n=%u tot=%ums max=%ums avg=%uus | DUMP n=%u "
                  "tot=%ums max=%ums avg=%uus | LEAK n=%u tot=%ums max=%ums "
                  "avg=%uus | WF n=%u tot=%ums max=%ums avg=%uus\n",
                  (unsigned)prof_pend_n, (unsigned)pend_ms, (unsigned)pend_mx,
                  (unsigned)pend_avg, (unsigned)prof_dump_n, (unsigned)dump_ms,
                  (unsigned)dump_mx, (unsigned)dump_avg, (unsigned)prof_leak_n,
                  (unsigned)leak_ms, (unsigned)leak_mx, (unsigned)leak_avg,
    ...
    if (currentState == SCREEN_CHART && currentRadioMode == RADIO_PCAP) {
      ...
      uint32_t win_x = (pcap_displayed_total >= prev_displayed)
                           ? pcap_displayed_total - prev_displayed
                           : pcap_displayed_total;
      uint32_t win_y = (pcap_upstream_total >= prev_upstream)
                           ? pcap_upstream_total - prev_upstream
                           : pcap_upstream_total;
      uint32_t win_z = (pcap_cooldown_total >= prev_cooldown)
                           ? pcap_cooldown_total - prev_cooldown
                           : pcap_cooldown_total;
